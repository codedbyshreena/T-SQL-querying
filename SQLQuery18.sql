...
HAVING AVG(SPI) = (SELECT MAX(AVGSPI) 
				   FROM 
						...
						FROM STUDENT 
						GROUP BY BRANCH) AS SPIBRANCH)

--13. Display the branch having minimum average SPI.  
SELECT BRANCH FROM STUDENT
GROUP BY BRANCH
HAVING AVG(SPI) = (SELECT MIN(AVGSPI) FROM 
										(SELECT AVG(SPI) AS AVGSPI 
										FROM STUDENT 
										GROUP BY BRANCH) AS SPIBRANCH)
...
JOIN RESULT R 
ON S.RNO = R.RNO
WHERE R.SPI > ALL (SELECT SPI FROM RESULT R1 
...
JOIN RESULT R 
ON S.RNO = R.RNO
WHERE R.SPI <> ANY (SELECT SPI FROM RESULT R1 
                   JOIN STUDENT_INFO S1 
				   ON S1.RNO = R1.RNO
				   WHERE BRANCH = 'EC')
...
JOIN RESULT R 
ON S.RNO = R.RNO
WHERE R.SPI > ALL (SELECT AVG(SPI) FROM RESULT R1 
                   JOIN STUDENT_INFO S1 
				   ON S1.BRANCH = S.BRANCH)
...
WHERE R.SPI > ALL (SELECT AVG(SPI) FROM RESULT R1 
				   ...
				   ON S1.RNO = R1.RNO
				   WHERE BRANCH = 'CE')
AND R.SPI > ALL (SELECT MAX(SPI) FROM RESULT R1 
                 JOIN STUDENT_INFO S1 
				 ON S1.RNO = R1.RNO
				 WHERE BRANCH = 'ME')

--20. Display the branch names whose average SPI is greater than the overall average SPI.
SELECT BRANCH, AVG(R.SPI) AS AVG_SPI FROM STUDENT_INFO S 
JOIN RESULT R 
ON S.RNO = R.RNO
GROUP BY BRANCH
HAVING AVG(R.SPI) > ALL (SELECT AVG(SPI) FROM RESULT R)

--21. Display the students who have maximum SPI in their respective branch. 
SELECT S.* FROM STUDENT_INFO S 
JOIN RESULT R 
ON S.RNO = R.RNO
WHERE R.SPI = ANY (SELECT MAX(SPI) AS MAX_SPI FROM RESULT R1 
                   JOIN STUDENT_INFO S1 
				   ON S1.RNO = R1.RNO
				   GROUP BY BRANCH)
...
WHERE R.SPI > ALL (SELECT AVG(SPI) FROM RESULT R1 
                   JOIN STUDENT_INFO S1 
				   ON S1.BRANCH = S.BRANCH)
AND R.SPI > ALL (SELECT AVG(SPI) FROM RESULT R1 
                 JOIN STUDENT_INFO S1 
				 ON S1.RNO = R1.RNO)
